 ...
 use POSIX qw/ceil/; #this is for function ceil()
  ...
  if (@ARGV != 2)
    { # @ARGV used in scalar context = number of args
	  print "the input could have 5 decimal, we write this test script to handle such case\n";
...
#          print "perl $0 ../result/Combined_predictions_for_CAFA1_v2 ../test/Re_ranged_combined_prediction_for_CAFA1_changed_rerange_script\n";
          print "perl $0 ../CAFA_2013_result/F_7_filtered_valid_prediction_by_MHS_and_minig_rules_CAFA2 ../CAFA_2013_result/F_8_reranged_filtered_valid_prediction_by_MHS_mining_rules_CAFA2\n";
  ...
  while(defined($line=<$IN>))
  {
	  ...
	  {#modelname targetname GO score
		  ...
		  {# already have some function
			  ...
			  {
				  print "The input prediction is not ranked! Check here! $line, $tem_split[3] and $tem2[3]. \n";
				  exit(0);
			  }
			  elsif($tem_split[3]<$tem2[3])
			  {
				  ...
				  if($tem2[1]<=0)
				  {
					  print "Warning, check why it decrease to negative??? $line, revise it to be 0.01\n";
					  #next;
					  $tem2[1]=0.01;
					  #exit(0);
				  }
